...
<div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
    <div class="mb-6">
        <h1 class="text-2xl font-bold text-gray-800">
            <?php echo e(isset($eventRequest) ? '✏️ Edit Event' : '📅 Tambah Event Baru'); ?>

        </h1>
    </div>

    <div class="bg-white shadow rounded-xl p-8 border border-gray-100">
        <form id="event-admin-form" method="POST" action="<?php echo e(isset($eventRequest) ? route('admin.event-requests.update', $eventRequest) : route('admin.event-requests.store')); ?>" class="space-y-6">
            <?php echo csrf_field(); ?>
            <?php if(isset($eventRequest)): ?> <?php echo method_field('PUT'); ?> <?php endif; ?>
            ...
            <div class="grid grid-cols-1 gap-6">
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        <label class="relative cursor-pointer">
                            <input type="radio" name="type" value="event" class="peer sr-only"
                                   <?php echo e(old('type', $eventRequest->type ?? 'event') === 'event' ? 'checked' : ''); ?>>
                        ...
                        <label class="relative cursor-pointer">
                            <input type="radio" name="type" value="disaster" class="peer sr-only"
                                   <?php echo e(old('type', $eventRequest->type ?? '') === 'disaster' ? 'checked' : ''); ?>>
                ...
                <div>
                    ...
                    <input type="text" name="event_name" value="<?php echo e(old('event_name', $eventRequest->event_name ?? '')); ?>" 
                ...
                <div>
                    ...
                    <textarea name="needs" rows="3" 
                              class="w-full rounded-lg border-gray-300 shadow-sm focus:border-emerald-500 focus:ring-emerald-500 font-medium" 
                              placeholder="Contoh: Butuh 1 Unit Ambulance dan 2 Tenaga Medis"><?php echo e(old('needs', $eventRequest->needs ?? '')); ?></textarea>
                </div>

                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        <label class="block text-sm font-bold text-gray-700 mb-1 uppercase tracking-wider">Tanggal Mulai</label>
                        <input type="date" name="start_date" value="<?php echo e(old('start_date', isset($eventRequest) ? $eventRequest->start_date->format('Y-m-d') : '')); ?>" 
                    ...
                    <div>
                        ...
                        <input type="date" name="end_date" value="<?php echo e(old('end_date', isset($eventRequest) ? $eventRequest->end_date->format('Y-m-d') : '')); ?>" 
                               class="w-full rounded-lg border-gray-300 shadow-sm focus:border-emerald-500 focus:ring-emerald-500 font-medium" required>
                    </div>
                </div>

                <?php if(isset($eventRequest)): ?>
                <div>
                    ...
                    <select name="status" class="w-full rounded-lg border-gray-300 shadow-sm focus:border-emerald-500 focus:ring-emerald-500 font-bold">
                        <option value="pending" <?php echo e($eventRequest->status === 'pending' ? 'selected' : ''); ?>>Pending</option>
                        <option value="approved" <?php echo e($eventRequest->status === 'approved' ? 'selected' : ''); ?>>Disetujui</option>
                        <option value="rejected" <?php echo e($eventRequest->status === 'rejected' ? 'selected' : ''); ?>>Ditolak</option>
                    </select>
                </div>
                <?php endif; ?>
            </div>
            ...
            <div class="pt-6 flex justify-between items-center gap-4">
                <a href="<?php echo e(route('admin.event-requests.index')); ?>" class="text-gray-500 hover:text-gray-700 font-bold">
                    ← Batal
                </a>
                <button type="submit" id="admin-submit-btn" class="bg-emerald-600 hover:bg-emerald-700 text-white px-8 py-3 rounded-xl font-bold shadow-lg transition transform active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed">
                    <span id="admin-btn-text"><?php echo e(isset($eventRequest) ? 'Simpan Perubahan' : 'Buat Event'); ?></span>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /Applications/Dev/ambulance-dispatch/resources/views/admin/event_requests/create.blade.php ENDPATH**/ ?>