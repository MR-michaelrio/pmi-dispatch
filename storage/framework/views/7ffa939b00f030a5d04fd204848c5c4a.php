<!DOCTYPE html>
<html lang="id">
<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <title>Jadwal Unit | GMCI</title>
    <script src="https://cdn.tailwindcss.com"></script>
    <meta http-equiv="refresh" content="60"> 
</head>
<body class="bg-gray-50 text-gray-900 antialiased min-h-screen">
    <header class="bg-white border-b shadow-sm py-4 mb-6">
        <div class="max-w-7xl mx-auto px-4 flex justify-between items-center">
            <div class="flex items-center gap-3">
                <img src="/logo.png" alt="GMCI Logo" class="h-8">
                <span class="font-bold text-gray-700">Jadwal Operasional GMCI</span>
            </div>
            <a href="<?php echo e(route('portal')); ?>" class="text-emerald-600 font-bold text-sm">← Kembali ke Portal</a>
        </div>
    </header>

    <main>
        <?php echo $__env->yieldContent('content'); ?>
    </main>

    <footer class="py-8 text-center text-xs text-gray-400">
        © <?php echo e(date('Y')); ?> Global Medical Care Indonesia
    </footer>
</body>
</html>
<?php /**PATH /Applications/Dev/ambulance-dispatch/resources/views/layouts/public_calendar.blade.php ENDPATH**/ ?>